...
// TRANSFORMER: This script converts Next Commerce dataLayer events prefixed with "dl_" to standard GA4 events. 
// Also swaps dl_accepted_upsell to purchase and sends to both dataLayer and gtag.
...
(function() {
  ...
  // Helper function to send event via gtag
  const sendToGtag = (eventName, eventData) => {
    // Check if gtag is available
    if (typeof gtag === 'undefined') {
      console.warn('[GA4 Bridge] gtag is not available yet');
      // Retry after a short delay if gtag isn't loaded yet
      setTimeout(() => {
        if (typeof gtag !== 'undefined') {
          sendToGtag(eventName, eventData);
        }
      }, 1000);
      return;
    }

    // Prepare gtag parameters - no need for send_to!
    const gtagParams = {};

    // Handle ecommerce events
    if (eventData.ecommerce) {
      // Add ecommerce parameters directly to gtag params
      Object.assign(gtagParams, {
      ...
      if (eventData.ecommerce.transaction_id) {
        gtagParams.transaction_id = eventData.ecommerce.transaction_id;
      }
      if (eventData.ecommerce.order_number) {
        gtagParams.order_number = eventData.ecommerce.order_number;
      }
      if (eventData.ecommerce.tax) {
        gtagParams.tax = eventData.ecommerce.tax;
      }
      if (eventData.ecommerce.shipping) {
        gtagParams.shipping = eventData.ecommerce.shipping;
      }
      if (eventData.ecommerce.coupon) {
        gtagParams.coupon = eventData.ecommerce.coupon;
      }
      if (eventData.ecommerce.affiliation) {
        gtagParams.affiliation = eventData.ecommerce.affiliation;
      }
    }

    // Add user properties if they exist
    if (eventData.user_properties) {
      gtagParams.user_properties = eventData.user_properties;
    }

    // Add any custom parameters (excluding internal ones)
    Object.keys(eventData).forEach(key => {
      if (!['event', 'ecommerce', 'user_properties', 'event_timestamp', 'event_id'].includes(key)) {
        gtagParams[key] = eventData[key];
      }
    });

    // Send to gtag - it will automatically send to all configured properties
    try {
      gtag('event', eventName, gtagParams);
      
      // Debug logging
      if (window.location.hostname === 'localhost' || window.location.search.includes('debug=true')) {
        console.log(`[GA4 Bridge] Sent to gtag: ${eventName}`, gtagParams);
      }
    } catch (error) {
      console.error('[GA4 Bridge] Error sending to gtag:', error);
    }
  };
  ...
  const initBridge = () => {
    ...
    console.log('[GA4 Bridge] Initializing dataLayer bridge with gtag support');
  ...
  const processEvent = (event) => {
    ...
    if (ECOMMERCE_EVENT_MAP[event.event] && (event.ecommerce || event.event === 'dl_accepted_upsell')) {
      // Clear ecommerce object first (GTM best practice for dataLayer)
      ...
      if (event.event === 'dl_accepted_upsell') {
        ...
        console.log(`[GA4 Bridge] Upsell converted to purchase with transaction_id: ${transactionId}, order_number: ${orderNumber}`, ga4Event);
      } 
    ...
    // Also send directly to gtag
    sendToGtag(ga4EventName, ga4Event);
  ...
  window.GA4Bridge = {
    ...
    isActive: () => true,
    testGtag: (eventName, eventData) => sendToGtag(eventName, eventData || { test: true })
  };
})();
