  ...
  (function() {
    ...
    const processEvent = (event) => {
      ...
      const eventId = `${event.event}_${event._metadata?.sequence_number || Date.now()}_${JSON.stringify(event.ecommerce?.items?.[0]?.item_id || '')}`;
      ...
      if (ECOMMERCE_EVENT_MAP[event.event] && event.ecommerce) {
      ...
      Object.keys(event).forEach(key => {
        if (!['event', 'ecommerce', '_metadata', 'event_timestamp', 'event_id', 'user_properties', 'method'].includes(key)) {
