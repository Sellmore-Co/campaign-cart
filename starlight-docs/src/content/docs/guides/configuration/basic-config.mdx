...
<Steps>
1. **JavaScript Configuration** (`window.osConfig`) - Highest priority, dynamic values
2. **Meta Tags** (`<meta name="os-*">`) - Page-level static configuration  
3. **Default Values** - Built-in fallbacks, lowest priority
</Steps>

<Aside type="note">
Higher priority configurations override lower ones. Use JavaScript for dynamic values and meta tags for static page settings.
</Aside>
...
<Tabs>
  <TabItem label="Meta Tags">
    <Code code={`<meta name="os-api-key" content="YOUR_API_KEY">
<meta name="os-campaign-id" content="YOUR_CAMPAIGN_ID">`} lang="html" title="Basic Meta Tag Setup" />
  </TabItem>
...
};`} lang="javascript" title="Basic JavaScript Setup" />
  </TabItem>
</Tabs>

<Aside type="caution">
Replace `YOUR_API_KEY` and `YOUR_CAMPAIGN_ID` with your actual credentials from the 29next dashboard.
</Aside>
