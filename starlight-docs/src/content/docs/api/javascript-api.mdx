...
<Code code={`window.twentyNineNext`} lang="javascript" />

<Aside type="tip">
All API methods are accessed through the `window.twentyNineNext` object. Make sure the SDK is loaded before calling any methods.
</Aside>
...
<Tabs>
  <TabItem label="Simple Add">
    <Code code={`window.twentyNineNext.addToCart(1);`} lang="javascript" />
  </TabItem>
  
  <TabItem label="With Quantity">
    <Code code={`window.twentyNineNext.addToCart(1, 2);`} lang="javascript" />
  </TabItem>
  
  <TabItem label="With Options">
    <Code code={`window.twentyNineNext.addToCart(1, 1, {
    upsell: true,
    frequency: 30
});`} lang="javascript" />
  </TabItem>
...
<Code code={`window.twentyNineNext.addToCartByRefId('CUSTOM-001', 1, 29.99, 'Custom Product');`} lang="javascript" />
