...
var TwentyNineNext = (() => {
  ...
  var UpsellManager = class {
    ...
    async acceptUpsell(packageIds, quantities, nextUrl) {
      ...
      if (!Array.isArray(packageIds) || !Array.isArray(quantities)) {
        __privateGet(this, _logger25).error("packageIds and quantities must be arrays");
        return;
      }
      if (packageIds.length === 0 || packageIds.length !== quantities.length) {
        __privateGet(this, _logger25).error("Invalid packageIds or quantities arrays");
        return;
      }
      __privateGet(this, _logger25).info(`Accepting upsell: Packages ${packageIds.join(", ")}, Quantities ${quantities.join(", ")}`);
      ...
      try {
        ...
        __privateMethod(this, _storeUpsellPurchaseData, storeUpsellPurchaseData_fn).call(this, response, packageIds, quantities);
    ...
    __privateGet(this, _upsellElements).acceptButtons.forEach((button) => {
      button.addEventListener("click", (event) => {
        ...
        if (packageIds) {
          ...
          const quantityArray = quantities ? quantities.split(",").map((q) => parseInt(q.trim(), 10)) : packageIdArray.map(() => 1);
          if (packageIdArray.length !== quantityArray.length) {
            __privateGet(this, _logger25).error("Mismatch between package IDs and quantities count");
            return;
          }
  ...
  storeUpsellPurchaseData_fn = function(response, packageIds, quantities) {
    try {
      ...
      if (lines.length === 0) {
        __privateGet(this, _logger25).warn("No valid packages found for upsell tracking");
        return;
      }
