import { B as BaseEnhancer } from "./BaseEnhancer-CAf7YqXf.js";
import { g as useAttributionStore, c as configStore, d as ApiClient, u as useCartStore, f as useCheckoutStore } from "./analytics-Dvf5RBLE.js";
...
class OrderBuilder {
  ...
  buildExpressOrder(cartItems, paymentMethod) {
    const orderData = {
      lines: cartItems.map((item) => ({
        package_id: item.packageId,
        quantity: item.quantity,
...
function handleOrderRedirect(order, logger, emitCallback) {
  let redirectUrl;
...
class OrderManager {
  ...
  async createOrder(checkoutFormData, cartItems, paymentMethod, paymentToken, billingAddress, sameAsShipping = true, shippingMethod, vouchers = [], resetCartCallback) {
  ...
  async createExpressOrder(cartItems, paymentMethod, resetCartCallback) {
  ...
  async createTestOrder(cartItems, resetCartCallback) {
...
class ExpressCheckoutProcessor {
  constructor(logger, addClassCallback, removeClassCallback, emitCallback, orderManager) {
    this.logger = logger;
    this.addClassCallback = addClassCallback;
    this.removeClassCallback = removeClassCallback;
    this.emitCallback = emitCallback;
    this.orderManager = orderManager;
  }
  async handleExpressCheckout(method, cartItems, isCartEmpty, resetCart) {
    try {
