import { e as createLogger, c as configStore, a as useCampaignStore, E as EventBus, u as useCartStore } from "./analytics-Dvf5RBLE.js";
...
const logger$5 = createLogger("PendingEventsHandler");
const STORAGE_KEY$1 = "next_pending_events";
class PendingEventsHandler {
  ...
  queueEvent(event) {
    try {
      ...
      logger$5.info(`Event queued for after redirect: ${event.event} (${pending.length} total queued)`);
    } catch (error) {
      logger$5.error("Failed to queue event:", error);
    }
  }
  /**
  ...
  getPendingEvents() {
    try {
      ...
      logger$5.error("Failed to get pending events:", error);
      return [];
    }
  }
  /**
  ...
  processPendingEvents() {
    ...
    if (events.length === 0) {
      logger$5.debug("No pending analytics events to process");
      return;
    }
    logger$5.info(`Processing ${events.length} pending analytics events`);
    const processedIds = [];
    for (const pendingEvent of events) {
      try {
        if (Date.now() - pendingEvent.timestamp > 5 * 60 * 1e3) {
          logger$5.warn("Skipping stale event:", pendingEvent.event.event);
          processedIds.push(pendingEvent.id);
          continue;
        }
        dataLayer.push(pendingEvent.event);
        processedIds.push(pendingEvent.id);
        logger$5.debug("Processed pending event:", pendingEvent.event.event);
      } catch (error) {
        logger$5.error("Failed to process pending event:", pendingEvent.event.event, error);
      }
    }
    if (processedIds.length > 0) {
      ...
      logger$5.debug("Removed processed events:", processedIds.length);
    }
  }
  /**
  ...
  clearPendingEvents() {
    try {
      sessionStorage.removeItem(STORAGE_KEY$1);
      logger$5.debug("Cleared all pending events");
    } catch (error) {
      logger$5.error("Failed to clear pending events:", error);
    }
  }
  /**
  ...
  reset() {
    ...
    logger$5.debug("PendingEventsHandler reset");
  }
  /**
   * Initialize the handler (called by NextAnalytics)
   */
  initialize() {
    logger$5.debug("PendingEventsHandler initialized");
  }
}
...
class ViewItemListTracker {
  ...
  trackViewItem(product) {
    ...
    logger$3.info("Tracked view_item:", product.packageId);
  }
  /**
  ...
  trackViewItemList(products) {
    ...
    dataLayer.push(event);
    logger$3.info(`Tracked view_item_list with ${items.length} items`);
  }
  /**
...
class UserDataTracker {
  ...
  trackUserData() {
    ...
    dataLayer.push(event);
    logger$2.info("Tracked user data:", {
...
class AutoEventListener {
  ...
  setupCartEventListeners() {
    const handleAddToCart = async (data) => {
      ...
      dataLayer.push(event);
      logger$1.info("Tracked add to cart:", packageId);
    };
    ...
    const handleRemoveFromCart = async (data) => {
      ...
      dataLayer.push(event);
      logger$1.info("Tracked remove from cart:", packageId);
    };
