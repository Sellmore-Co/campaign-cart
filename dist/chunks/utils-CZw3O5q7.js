...
const campaignStoreInstance = create((set, get) => ({
  ...
  loadCampaign: async (apiKey) => {
    ...
    try {
      ...
      if (cachedData && cachedData.apiKey === apiKey && now - cachedData.timestamp < CACHE_EXPIRY_MS) {
        logger$b.info("🎯 Using cached campaign data (expires in " + Math.round((CACHE_EXPIRY_MS - (now - cachedData.timestamp)) / 1e3) + " seconds)");
      ...
      logger$b.info("🌐 Fetching fresh campaign data from API...");
      const { ApiClient } = await import("./api-CKvZdO89.js");
      ...
      logger$b.info("💾 Campaign data cached for 5 minutes");
      set({
        data: campaign,
  ...
  clearCache: () => {
    ...
    logger$b.info("🗑️ Campaign cache cleared");
  },
  getCacheInfo: () => {
    const cachedData = sessionStorageManager.get(CAMPAIGN_STORAGE_KEY);
    if (!cachedData) {
