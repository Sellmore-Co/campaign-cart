...
const logger$c = createLogger("CampaignStore");
...
const campaignStoreInstance = create((set, get) => ({
  ...
  loadCampaign: async (apiKey) => {
    ...
    try {
      ...
      if (cachedData && cachedData.apiKey === apiKey && now - cachedData.timestamp < CACHE_EXPIRY_MS) {
        logger$c.info("🎯 Using cached campaign data (expires in " + Math.round((CACHE_EXPIRY_MS - (now - cachedData.timestamp)) / 1e3) + " seconds)");
      ...
      logger$c.info("🌐 Fetching fresh campaign data from API...");
      const { ApiClient } = await import("./api-wdEtYTpc.js");
      ...
      if (campaign.payment_env_key) {
        ...
        logger$c.info("💳 Spreedly environment key updated from campaign API: " + campaign.payment_env_key);
      }
      const cacheData = {
        campaign,
        timestamp: now,
      ...
      logger$c.info("💾 Campaign data cached for 5 minutes");
      set({
        data: campaign,
  ...
  clearCache: () => {
    ...
    logger$c.info("🗑️ Campaign cache cleared");
  },
  getCacheInfo: () => {
    const cachedData = sessionStorageManager.get(CAMPAIGN_STORAGE_KEY);
    if (!cachedData) {
