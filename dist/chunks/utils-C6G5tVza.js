    ...
    subscribeWithSelector((set, get) => ({
      ...
      addItem: async (item) => {
        ...
        set((state) => {
          ...
          if (item.isUpsell) {
            logger$b.debug(`Adding upsell item:`, {
      ...
      syncWithAPI: async () => {
        logger$b.debug("syncWithAPI not yet implemented");
      },
      calculateTotals: async () => {
        try {
          ...
          for (const appliedCoupon of appliedCoupons) {
            ...
            if (coupon?.scope === "package" && Array.isArray(coupon.packageIds) && coupon.packageIds.length > 0) {
              for (const packageId of coupon.packageIds) {
                const eligibleItems = state.items.filter((item) => item.packageId === packageId);
                const packageTotal = eligibleItems.reduce((sum, item) => sum + item.price * item.quantity, 0);
                if (packageTotal > 0) {
                  let discount = 0;
                  if (coupon.type === "percentage") {
                    discount = packageTotal * (coupon.value / 100);
                    if (coupon.maxDiscount) {
                      discount = Math.min(discount, coupon.maxDiscount);
                    }
                  } else {
                    discount = Math.min(coupon.value, packageTotal);
                  }
                  packageDiscounts[packageId] = (packageDiscounts[packageId] || 0) + discount;
                }
              }
            }
          }
          const subtotal = state.items.reduce((sum, item) => {
            ...
            const itemTotal = Math.max(0, price * quantity);
            const packageDiscount = packageDiscounts[item.packageId] || 0;
            const samePackageTotal = state.items.filter((i) => i.packageId === item.packageId).reduce((s, i) => {
              const p = parseFloat(String(i.price)) || 0;
              const q = parseInt(String(i.quantity)) || 0;
              return s + p * q;
            }, 0);
            const itemDiscountPortion = samePackageTotal > 0 && !isNaN(samePackageTotal) ? Math.min(itemTotal, itemTotal / samePackageTotal * packageDiscount) : 0;
            return sum + Math.max(0, itemTotal - itemDiscountPortion);
          }, 0);
          ...
          const retailSavings = Math.max(0, compareTotal - originalSubtotal);
          const savingsPercentage = compareTotal > 0 && !isNaN(compareTotal) ? Math.min(100, Math.max(0, retailSavings / compareTotal * 100)) : 0;
          const hasSavings = retailSavings > 0.01;
          const shipping = get().calculateShipping();
          const tax = get().calculateTax();
          let orderLevelDiscounts = 0;
          const totalPackageDiscounts = Object.values(packageDiscounts).reduce((sum, d) => sum + d, 0);
          const updatedCoupons = appliedCoupons.map((appliedCoupon) => {
            const coupon = appliedCoupon.definition;
            let discountAmount = 0;
            if (coupon.scope === "order") {
              const originalSubtotal2 = state.items.reduce((sum, item) => sum + item.price * item.quantity, 0);
              if (coupon.type === "percentage") {
                discountAmount = originalSubtotal2 * (coupon.value / 100);
                if (coupon.maxDiscount) {
                  discountAmount = Math.min(discountAmount, coupon.maxDiscount);
                }
              } else {
                discountAmount = Math.min(coupon.value, originalSubtotal2);
              }
              orderLevelDiscounts += discountAmount;
            } else if (coupon.scope === "package" && coupon.packageIds) {
              discountAmount = coupon.packageIds.reduce((sum, pkgId) => sum + (packageDiscounts[pkgId] || 0), 0);
            }
            return {
              ...appliedCoupon,
          ...
          const safeShipping = isNaN(shipping) ? 0 : Math.max(0, shipping);
          const safeTax = isNaN(tax) ? 0 : Math.max(0, tax);
          const safeOrderDiscounts = isNaN(orderLevelDiscounts) ? 0 : Math.max(0, orderLevelDiscounts);
          const total = Math.max(0, safeSubtotal + safeShipping + safeTax - safeOrderDiscounts);
          const totalExclShipping = Math.max(0, safeSubtotal + safeTax - safeOrderDiscounts);
          const allDiscounts = safeOrderDiscounts + totalPackageDiscounts;
          const totalSavings = Math.max(0, retailSavings + allDiscounts);
          const totalSavingsPercentage = compareTotal > 0 && !isNaN(compareTotal) ? Math.min(100, Math.max(0, totalSavings / compareTotal * 100)) : 0;
          const hasTotalSavings = totalSavings > 0.01;
          const totals = {
            ...
            discounts: { value: orderLevelDiscounts, formatted: formatCurrency(orderLevelDiscounts) },
            // Only show order-level discounts separately
      ...
      calculateEnrichedItems: async () => {
        try {
          ...
          const enrichedItems = state.items.map((item) => {
            ...
            const originalLineTotal = packagePrice * item.quantity;
            const packageDiscount = packageDiscounts[item.packageId] || 0;
            const samePackageTotal = state.items.filter((i) => i.packageId === item.packageId).reduce((s, i) => {
              const p = parseFloat(String(i.price)) || 0;
              const q = parseInt(String(i.quantity)) || 0;
              return s + Math.max(0, p * q);
            }, 0);
            const itemDiscountPortion = samePackageTotal > 0 && !isNaN(samePackageTotal) ? Math.min(originalLineTotal, originalLineTotal / samePackageTotal * packageDiscount) : 0;
            const lineTotal = Math.max(0, originalLineTotal - itemDiscountPortion);
            const pkgQty = parseInt(String(packageData?.qty)) || 1;
            const itemQty = parseInt(String(item.quantity)) || 1;
            const discountedUnitPrice = itemQty > 0 && pkgQty > 0 ? lineTotal / (itemQty * pkgQty) : actualUnitPrice;
            ...
            const unitSavings = Math.max(0, retailUnitPrice - discountedUnitPrice);
            const lineSavings = Math.max(0, retailLineTotal - lineTotal);
            const savingsPct = retailUnitPrice > discountedUnitPrice ? Math.round(unitSavings / retailUnitPrice * 100) : 0;
            ...
            return {
              ...
              price: {
                excl_tax: { value: discountedUnitPrice, formatted: formatCurrency(discountedUnitPrice) },
                incl_tax: { value: discountedUnitPrice, formatted: formatCurrency(discountedUnitPrice) },
                ...
                // Track if package discount was applied
                hasPackageDiscount: itemDiscountPortion > 0,
                packageDiscountAmount: { value: itemDiscountPortion, formatted: formatCurrency(itemDiscountPortion) }
              },
              product: {
                title: item.title || packageData?.name || "",
              ...
              hasComparePrice: retailUnitPrice > discountedUnitPrice,
...
const campaignStoreInstance = create((set, get) => ({
  ...
  loadCampaign: async (apiKey) => {
    ...
    try {
      ...
      if (cachedData && cachedData.apiKey === apiKey && now - cachedData.timestamp < CACHE_EXPIRY_MS) {
        logger$a.info("🎯 Using cached campaign data (expires in " + Math.round((CACHE_EXPIRY_MS - (now - cachedData.timestamp)) / 1e3) + " seconds)");
      ...
      logger$a.info("🌐 Fetching fresh campaign data from API...");
      const { ApiClient } = await import("./api-97GCV_NK.js");
      ...
      logger$a.info("💾 Campaign data cached for 5 minutes");
      set({
        data: campaign,
  ...
  clearCache: () => {
    ...
    logger$a.info("🗑️ Campaign cache cleared");
  },
  getCacheInfo: () => {
    const cachedData = sessionStorageManager.get(CAMPAIGN_STORAGE_KEY);
    if (!cachedData) {
...
const logger$9 = createLogger("AttributionCollector");
class AttributionCollector {
  ...
  getFunnelName() {
    try {
      ...
      if (sessionFunnel) {
        logger$9.debug(`Using persisted funnel from session: ${sessionFunnel}`);
        return sessionFunnel;
      }
      const localFunnel = localStorage.getItem("next_funnel_name");
      if (localFunnel) {
        logger$9.debug(`Using persisted funnel from localStorage: ${localFunnel}`);
      ...
      if (persistedData) {
        ...
        if (parsed.state && parsed.state.funnel) {
          logger$9.debug(`Using persisted funnel from attribution: ${parsed.state.funnel}`);
    ...
    if (funnelMetaTag) {
      ...
      if (value) {
        logger$9.debug(`New funnel found from meta tag: ${value}`);
        try {
          sessionStorage.setItem("next_funnel_name", value);
          localStorage.setItem("next_funnel_name", value);
          logger$9.info(`Persisted funnel name: ${value}`);
        } catch (error) {
  ...
  handleEverflowClickId(metadata) {
    ...
    if (urlParams.has("evclid")) {
      ...
      localStorage.setItem("evclid", evclid);
      sessionStorage.setItem("evclid", evclid);
      logger$9.debug(`Everflow click ID found in URL: ${evclid}`);
    } else if (!evclid && sessionStorage.getItem("evclid")) {
      evclid = sessionStorage.getItem("evclid");
      if (evclid) {
        localStorage.setItem("evclid", evclid);
        logger$9.debug(`Everflow click ID found in sessionStorage: ${evclid}`);
      }
    }
    if (urlParams.has("sg_evclid")) {
      ...
      logger$9.debug(`SG Everflow click ID found: ${sg_evclid}`);
    } else {
    ...
    if (evclid) {
      ...
      logger$9.debug(`Added Everflow transaction ID to metadata: ${evclid}`);
    }
  }
  /**
  ...
  collectTrackingTags(metadata) {
    ...
    logger$9.debug(`Found ${trackingTags.length} tracking tags`);
    trackingTags.forEach((tag) => {
      ...
      if (tagName && tagValue) {
        ...
        logger$9.debug(`Added tracking tag: ${tagName} = ${tagValue}`);
        if (shouldPersist) {
          try {
            sessionStorage.setItem(`tn_tag_${tagName}`, tagValue);
            logger$9.debug(`Persisted tracking tag: ${tagName}`);
          } catch (error) {
  ...
  getFacebookPixelId() {
    ...
    if (pixelMeta) {
      ...
      if (pixelId) {
        logger$9.debug(`Facebook Pixel ID found from meta tag: ${pixelId}`);
        return pixelId;
      }
    }
    ...
    for (const script of scripts) {
      ...
      if (content.includes("fbq(") && content.includes("init")) {
        ...
        if (match && match[1]) {
          logger$9.debug(`Facebook Pixel ID found from script: ${match[1]}`);
          return match[1];
        }
      }
    }
...
const logger$4 = createLogger("UserDataStorage");
class UserDataStorage {
    ...
    try {
      ...
      if (cookieData) {
        try {
          ...
          logger$4.debug("Loaded user data from cookie:", {
            hasEmail: !!this.userData.email,
            hasUserId: !!this.userData.userId
          });
        } catch (error) {
          logger$4.warn("Failed to parse user data cookie:", error);
        }
      }
      ...
      if (sessionData) {
        try {
          ...
          logger$4.debug("Merged user data from sessionStorage");
        } catch (error) {
          logger$4.warn("Failed to parse sessionStorage user data:", error);
        }
      }
      ...
      logger$4.error("Failed to load user data:", error);
    }
  }
  /**
  ...
  saveUserData() {
    ...
    try {
      ...
      logger$4.debug("Saved user data to storage:", {
        hasEmail: !!this.userData.email,
        hasUserId: !!this.userData.userId
      });
    } catch (error) {
      logger$4.error("Failed to save user data:", error);
    }
  }
  /**
  ...
  updateUserData(data) {
    ...
    if (data.email && data.email !== previousEmail) {
      logger$4.info("User email updated:", data.email);
    }
  }
  /**
   * Get all user data
  ...
  clearUserData() {
    ...
    logger$4.info("User data cleared");
  }
  /**
  ...
  updateFromFormFields() {
    ...
    if (hasUpdates) {
      ...
      logger$4.debug("Updated user data from form fields:", updates);
    }
  }
}
...
class UserDataTracker {
  ...
  initialize() {
    ...
    this.trackUserData();
    this.setupListeners();
    logger$3.info("UserDataTracker initialized");
  }
  /**
  ...
  setupListeners() {
    ...
    this.eventBus.on("cart:updated", () => {
      logger$3.debug("Cart updated, tracking user data");
      this.trackUserData();
    });
    const unsubscribeCart = useCartStore.subscribe(() => {
      logger$3.debug("Cart store changed, tracking user data");
      this.trackUserData();
    });
...
class AutoEventListener {
  ...
  setupUpsellEventListeners() {
    ...
    const handleUpsellAccepted = async (data) => {
      ...
      logger$2.debug("Upsell accepted event data:", { willRedirect: data.willRedirect, data });
      if (data.willRedirect) {
        acceptedUpsellEvent._willRedirect = true;
        logger$2.debug("Marked upsell event for queueing due to redirect");
      }
      dataLayer.push(acceptedUpsellEvent);
      logger$2.info("Tracked upsell accepted:", packageId);
    };
  ...
  setupCheckoutEventListeners() {
    const handleCheckoutStarted = async (data) => {
      ...
      const campaignStore2 = useCampaignStore.getState();
      const items = cartStore2.items.map((item, index2) => {
      ...
      const event = dataLayer.formatEcommerceEvent("dl_begin_checkout", {
        currency: campaignStore2.data?.currency || "USD",
        value: cartStore2.total || cartStore2.subtotal || 0,
        items,
        coupon: data.coupon
      });
      dataLayer.push(event);
      logger$2.info("Tracked checkout started");
    };
    ...
    const handleExpressCheckoutStarted = async (data) => {
      ...
      const event = dataLayer.formatEcommerceEvent("dl_begin_checkout", {
        ...
        value: cartStore2.total || cartStore2.subtotal || 0,
        items,
        coupon: cartStore2.appliedCoupons?.[0]?.code,
        payment_type: data.method || "express"
      });
      dataLayer.push(event);
      logger$2.info("Tracked express checkout started", { method: data.method });
    };
    ...
    const handleOrderCompleted = async (order) => {
      ...
      if (order.lines && Array.isArray(order.lines)) {
        ...
        const cartStore2 = useCartStore.getState();
        const campaignStore2 = useCampaignStore.getState();
        items = cartStore2.items.map((item, index2) => {
          const packageData = campaignStore2.getPackage(item.packageId);
          return {
            item_id: packageData?.external_id?.toString() || item.packageId.toString(),
            // Use external_id for analytics
...
const logger$1 = createLogger("NextAnalytics");
class NextAnalytics {
  constructor() {
  ...
  checkAndSetIgnoreFlag() {
    ...
    try {
      ...
      if (ignoreParam === "true") {
        ...
        logger$1.info("Analytics ignore flag set from URL parameter");
      }
    } catch (error) {
      logger$1.error("Error checking ignore parameter:", error);
    }
  }
  /**
  ...
  shouldIgnoreAnalytics() {
    ...
    try {
      ...
      logger$1.error("Error checking ignore status:", error);
      return false;
    }
  }
  /**
  ...
  async initialize() {
    if (this.initialized) {
      logger$1.debug("Analytics already initialized");
      return;
    }
    if (this.shouldIgnoreAnalytics()) {
      logger$1.info("Analytics ignored due to ignore parameter");
      return;
    }
    try {
      const config = configStore.getState();
      if (!config.analytics?.enabled) {
        logger$1.info("Analytics disabled in configuration");
        return;
      }
      dataLayer.initialize();
      ...
      logger$1.info("NextAnalytics initialized successfully", {
        providers: Array.from(this.providers.keys()),
        mode: config.analytics.mode
      });
    } catch (error) {
      logger$1.error("Failed to initialize analytics:", error);
      throw error;
    }
  }
  /**
  ...
  async initializeProviders(config, storeName) {
    if (config.providers?.nextCampaign?.enabled) {
      ...
      logger$1.info("NextCampaign adapter initialized");
    }
    if (config.providers?.gtm?.enabled) {
      ...
      logger$1.info("GTM adapter initialized");
    }
    if (config.providers?.facebook?.enabled && config.providers.facebook.settings?.pixelId) {
      ...
      logger$1.info("Facebook Pixel adapter initialized", {
    ...
    if (config.providers?.rudderstack?.enabled) {
      ...
      logger$1.info("RudderStack adapter initialized");
    }
    if (config.providers?.custom?.enabled && config.providers.custom.settings?.endpoint) {
      ...
      logger$1.info("Custom adapter initialized");
    }
  }
  /**
  ...
  initializeAutoTracking() {
    ...
    logger$1.info("Auto-tracking initialized");
  }
  /**
   * Track an event
   */
  track(event) {
    if (this.shouldIgnoreAnalytics()) {
      logger$1.debug("Event tracking skipped due to ignore flag:", event.event);
      return;
    }
    if (!this.initialized) {
      logger$1.warn("Analytics not initialized, queuing event:", event.event);
    }
    if (dataLayer.isDebugMode()) {
      const validation = this.validator.validateEvent(event);
      if (!validation.valid) {
        logger$1.error("Event validation failed:", validation.errors);
        if (validation.warnings.length > 0) {
          logger$1.warn("Event validation warnings:", validation.warnings);
        }
      }
    }
    dataLayer.push(event);
  ...
  setDebugMode(enabled) {
    ...
    logger$1.info(`Debug mode ${enabled ? "enabled" : "disabled"}`);
  }
  /**
  ...
  clearIgnoreFlag() {
    if (typeof window !== "undefined") {
      try {
        ...
        logger$1.info("Analytics ignore flag cleared");
      } catch (error) {
        logger$1.error("Error clearing ignore flag:", error);
      }
    }
  }
  /**
...
export {
  ...
  configStore as e,
  LogLevel as f,
  ElementDataExtractor as g,
  PriceCalculator as h,
  EcommerceEvents as i,
  ErrorDisplayManager as j,
  EventHandlerManager as k,
  userDataStorage as l,
  getFailureUrl as m,
  nextAnalytics as n,
  getSuccessUrl as o,
  handleOrderRedirect as p,
  preserveQueryParams as q,
  cartStore as r,
  index as s,
