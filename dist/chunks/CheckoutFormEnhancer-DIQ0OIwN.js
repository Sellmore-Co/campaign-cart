import { B as BaseEnhancer } from "./BaseEnhancer-D7xiymS-.js";
import { c as createLogger, F as FieldFinder, d as useCheckoutStore, n as nextAnalytics, m as EcommerceEvents, o as ErrorDisplayManager, p as EventHandlerManager, f as configStore, u as useCartStore, b as useAttributionStore, a as useCampaignStore, C as CountryService, q as userDataStorage } from "./utils-C9a5e9jY.js";
...
import { O as OrderManager, E as ExpressCheckoutProcessor } from "./OrderManager-BYA_d-MI.js";
...
class CheckoutFormEnhancer extends BaseEnhancer {
  constructor(element) {
    ...
    this.lastCurrencyChangeCountry = null;
    this.lastCurrencyChangeTime = 0;
  ...
  async handleCountryChange(newCountry) {
    ...
    if (countryField instanceof HTMLSelectElement) {
      ...
      countryField._externalChange = true;
      countryField.dispatchEvent(new Event("change", { bubbles: true }));
      setTimeout(() => {
        delete countryField._externalChange;
      }, 100);
    ...
    if (billingCountryField instanceof HTMLSelectElement) {
      ...
      billingCountryField._externalChange = true;
      billingCountryField.dispatchEvent(new Event("change", { bubbles: true }));
      setTimeout(() => {
        delete billingCountryField._externalChange;
      }, 100);
    }
  }
  ...
  async handleCountryCurrencyChange(countryCode, field) {
    if (field && field._externalChange) {
      this.logger.debug("Skipping currency auto-switch for external country change");
      return;
    }
    const now = Date.now();
    if (this.lastCurrencyChangeCountry === countryCode && now - this.lastCurrencyChangeTime < 500) {
    ...
    try {
      const configStore$1 = configStore.getState();
      if (configStore$1.currencyBehavior === "manual") {
      ...
      const currencyAvailable = availableCurrencies.some((c) => c.code === newCurrency);
      if (!currencyAvailable) {
        this.logger.debug(`Currency ${newCurrency} not available in campaign for country ${countryCode}`);
        return;
      }
      const currentCurrency = campaignStore.data?.currency || configStore$1.selectedCurrency || "USD";
      ...
      const { CurrencyFormatter } = await import("./utils-C9a5e9jY.js").then((n) => n.H);
