import { c as create, p as persist, s as subscribeWithSelector, d as devtools } from "./vendor-B2NPMtAQ.js";
...
const logger$d = createLogger("CampaignStore");
  ...
  loadCampaign: async (apiKey) => {
    ...
    try {
      ...
      if (!cachedData && requestedCurrency !== "USD") {
        ...
        if (cachedData) {
          logger$d.info(`No cache for ${requestedCurrency}, checking USD cache as potential fallback`);
        }
      }
      if (cachedData && cachedData.apiKey === apiKey && now - cachedData.timestamp < CACHE_EXPIRY_MS) {
        ...
        logger$d.info(`🎯 Using cached campaign data for ${cachedCurrency} (expires in ` + Math.round((CACHE_EXPIRY_MS - (now - cachedData.timestamp)) / 1e3) + " seconds)");
        if (cachedCurrency !== requestedCurrency) {
          logger$d.warn(`⚠️ Requested ${requestedCurrency} but using cached ${cachedCurrency} (fallback)`);
      ...
      logger$d.info(`🌐 Fetching campaign data from API with currency: ${requestedCurrency}...`);
      const { ApiClient } = await import("./api-D4lLxLPI.js");
      ...
      if (actualCurrency !== requestedCurrency) {
        logger$d.warn(`⚠️ API Fallback: Requested ${requestedCurrency}, received ${actualCurrency}`);
        configStore2.updateConfig({
      ...
      if (campaign.payment_env_key) {
        ...
        logger$d.info("💳 Spreedly environment key updated from campaign API: " + campaign.payment_env_key);
      }
      ...
      logger$d.info(`💾 Campaign data cached for ${actualCurrency} (10 minutes)`);
      if (actualCurrency !== requestedCurrency) {
        sessionStorage.removeItem(requestedCacheKey);
        logger$d.debug(`Cleared invalid cache for ${requestedCurrency}`);
      }
      set({
        data: campaign,
      ...
      if (!cartStore$1.isEmpty && cartStore$1.lastCurrency && cartStore$1.lastCurrency !== actualCurrency) {
        logger$d.info("Currency changed, refreshing cart prices...");
      ...
      logger$d.error("Campaign load failed:", error);
      throw error;
    }
  },
  getPackage: (id) => {
  ...
  clearCache: () => {
    try {
      ...
      keysToRemove.forEach((key) => {
        ...
        logger$d.debug(`Removed cache: ${key}`);
      });
      logger$d.info(`🗑️ Campaign cache cleared (${keysToRemove.length} entries removed)`);
    } catch (error) {
      logger$d.error("Failed to clear campaign cache:", error);
  ...
  persist(
    subscribeWithSelector((set, get) => ({
      ...
      swapPackage: async (removePackageId, addItem) => {
        const { useCampaignStore: useCampaignStore2 } = await Promise.resolve().then(() => campaignStore);
        const campaignStore$1 = useCampaignStore2.getState();
        const newPackageData = campaignStore$1.getPackage(addItem.packageId ?? 0);
        if (!newPackageData) {
        ...
        const newItem = {
          ...
          image: addItem.image ?? void 0,
          sku: addItem.sku ?? void 0,
        ...
        try {
          ...
          if (updatedShippingMethod && campaignStore$1.data.shipping_methods) {
            ...
            if (shippingMethodData) {
              ...
              logger$c.info(`Updated shipping method price: ${updatedShippingMethod.code} = ${newPrice} ${campaignStore$1.data.currency}`);
            }
          }
          set((state2) => {
            const updates = {
...
const logger$b = createLogger("OrderStore");
  ...
  devtools(
    persist(
      (set, get) => ({
        ...
        // Order management
        setOrder: (order) => {
          logger$b.debug("Setting order data:", order);
          set({
            order,
            error: null,
        ...
        setRefId: (refId) => {
          logger$b.debug("Setting ref ID:", refId);
          set({ refId });
        },
        loadOrder: async (refId, apiClient) => {
          const state = get();
          if (state.order && state.refId === refId && !get().isOrderExpired()) {
            logger$b.info("Using cached order data:", refId);
            return;
          }
          if (state.isLoading) {
            logger$b.warn("Order loading already in progress");
            return;
          }
          logger$b.info("Loading order:", refId);
          set({ isLoading: true, error: null, refId });
          try {
            const order = await apiClient.getOrder(refId);
            logger$b.info("Order loaded successfully:", order);
            ...
            if (order.lines && Array.isArray(order.lines)) {
              order.lines.forEach((line) => {
                if (line.is_upsell && line.product_sku) {
                  ...
                  logger$b.debug("Detected upsell line:", {
            ...
            logger$b.debug("Populated completed upsells from order:", upsellPackageIds);
          } catch (error) {
            const errorMessage = error instanceof Error ? error.message : "Failed to load order";
            logger$b.error("Failed to load order:", error);
            set({
              isLoading: false,
              error: errorMessage,
              order: null
        ...
        clearOrder: () => {
          logger$b.debug("Clearing order data");
          set({
            order: null,
            refId: null,
            error: null,
        ...
        isOrderExpired: () => {
          ...
          if (isExpired) {
            logger$b.info("Order data has expired (>15 minutes old)");
          }
          return isExpired;
        },
        // Upsell management
        addUpsell: async (upsellData, apiClient) => {
          ...
          if (state.isProcessingUpsell) {
            logger$b.warn("Upsell processing already in progress");
            return null;
          }
          logger$b.info("Adding upsell to order:", state.refId, upsellData);
          set({ isProcessingUpsell: true, upsellError: null });
          try {
            const updatedOrder = await apiClient.addUpsell(state.refId, upsellData);
            logger$b.info("Upsell added successfully:", updatedOrder);
            ...
            logger$b.error("Failed to add upsell:", error);
            set({
              isProcessingUpsell: false,
        ...
        addPendingUpsell: (upsellData) => {
          const state = get();
          logger$b.debug("Adding pending upsell:", upsellData);
          set({
        ...
        removePendingUpsell: (index2) => {
          ...
          logger$b.debug("Removing pending upsell at index:", index2);
          set({ pendingUpsells: newPendingUpsells });
        },
        clearPendingUpsells: () => {
          logger$b.debug("Clearing pending upsells");
          set({ pendingUpsells: [] });
        },
        markUpsellCompleted: (packageId) => {
          const state = get();
          if (!state.completedUpsells.includes(packageId)) {
            logger$b.debug("Marking upsell as completed:", packageId);
            set({
        ...
        markUpsellViewed: (packageId) => {
          ...
          if (!state.viewedUpsells.includes(packageId)) {
            logger$b.debug("Marking upsell as viewed:", packageId);
            const journeyEntry = {
              packageId,
              action: "viewed",
              timestamp: Date.now()
        ...
        markUpsellPageViewed: (pagePath) => {
          ...
          if (!state.viewedUpsellPages.includes(pagePath)) {
            logger$b.debug("Marking upsell page as viewed:", pagePath);
            const journeyEntry = {
              pagePath,
              action: "viewed",
              timestamp: Date.now()
        ...
        markUpsellSkipped: (packageId, pagePath) => {
          const state = get();
          logger$b.debug("Marking upsell as skipped:", { packageId, pagePath });
          const journeyEntry = {
            action: "skipped",
            timestamp: Date.now()
          };
        ...
        reset: () => {
          logger$b.debug("Resetting order store");
          set(initialState$1);
        }
      }),
      {
...
class PendingEventsHandler {
  ...
  processPendingEvents() {
    ...
    const processedIds = [];
    for (const pendingEvent of events2) {
      try {
...
const _EventTimelinePanel = class _EventTimelinePanel {
  ...
  saveEvents() {
    ...
    this.saveTimeout = setTimeout(() => {
      try {
        const eventsToSave = this.events.slice(0, _EventTimelinePanel.MAX_STORED_EVENTS);
        localStorage.setItem(_EventTimelinePanel.EVENTS_STORAGE_KEY, JSON.stringify(eventsToSave));
      } catch (error) {
  ...
  watchDOMEvents() {
    ...
    EventTarget.prototype.dispatchEvent = function(event) {
      if (event instanceof CustomEvent && !eventsToWatch.includes(event.type) && !eventsToIgnore.includes(event.type) && !event.type.startsWith("debug:")) {
...
_EventTimelinePanel.MAX_STORED_EVENTS = 500;
...
export {
  ...
  useOrderStore as e,
  configStore as f,
  CART_STORAGE_KEY as g,
  LogLevel as h,
  getCurrencySymbol as i,
  formatCurrency as j,
  ElementDataExtractor as k,
  PriceCalculator as l,
  EcommerceEvents as m,
  nextAnalytics as n,
  ErrorDisplayManager as o,
  EventHandlerManager as p,
  userDataStorage as q,
  getPaymentCapabilities as r,
  isApplePayAvailable as s,
  testModeManager as t,
  useCartStore as u,
  isPayPalAvailable as v,
  isGooglePayAvailable as w,
  getFailureUrl as x,
  getSuccessUrl as y,
