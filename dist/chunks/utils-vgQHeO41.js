      ...
      addItem: async (item) => {
        ...
        if (!item.originalPackageId && profileStore$1.activeProfileId) {
          ...
          if (mappedId !== finalPackageId) {
            logger$e.debug(`Applying profile mapping: ${finalPackageId} -> ${mappedId}`);
...
const campaignStoreInstance = create((set, get) => ({
  ...
  loadCampaign: async (apiKey) => {
    ...
    try {
      ...
      const { ApiClient } = await import("./api-DtVFHPSI.js");
...
const logger$b = createLogger("ProfileStore");
    ...
    subscribeWithSelector((set, get) => ({
      ...
      registerProfile: (profile) => {
        set((state) => {
          ...
          logger$b.info(`Profile "${profile.id}" registered with ${Object.keys(profile.packageMappings).length} mappings`);
          return { profiles };
        });
      },
      activateProfile: (profileId) => {
        ...
        if (!state.profiles.has(profileId)) {
          logger$b.error(`Cannot activate profile "${profileId}" - not found`);
          return;
        }
        set({
        ...
        logger$b.info(`Profile "${profileId}" activated`);
      },
      deactivateProfile: () => {
        const state = get();
        set({
        ...
        logger$b.info("Profile deactivated");
      },
      getMappedPackageId: (originalId) => {
        ...
        if (mappedId !== void 0) {
          logger$b.debug(`Mapped package ${originalId} -> ${mappedId} (profile: ${state.activeProfileId})`);
          return mappedId;
        }
        return originalId;
      },
      ...
      setOriginalCartSnapshot: (items) => {
        ...
        logger$b.debug(`Cart snapshot saved with ${items.length} items`);
      },
      clearOriginalCartSnapshot: () => {
        set({ originalCartSnapshot: void 0 });
        logger$b.debug("Cart snapshot cleared");
      },
      ...
      reset: () => {
        set(initialState$2);
        logger$b.info("ProfileStore reset to initial state");
      }
    })),
    {
...
export {
  ...
  configStore as f,
  LogLevel as g,
  useProfileStore as h,
  PackageContextResolver as i,
  ElementDataExtractor as j,
  PriceCalculator as k,
  EcommerceEvents as l,
  ErrorDisplayManager as m,
  nextAnalytics as n,
  EventHandlerManager as o,
  CountryService as p,
