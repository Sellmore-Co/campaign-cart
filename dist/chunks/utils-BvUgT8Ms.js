...
const campaignStoreInstance = create((set, get) => ({
  ...
  loadCampaign: async (apiKey) => {
    ...
    try {
      ...
      const { ApiClient } = await import("./api-DoFAh-_c.js");
...
const logger$8 = createLogger("AttributionCollector");
class AttributionCollector {
  ...
  getFunnelName() {
    try {
      ...
      if (sessionFunnel) {
        logger$8.debug(`Using persisted funnel from session: ${sessionFunnel}`);
        return sessionFunnel;
      }
      const localFunnel = localStorage.getItem("next_funnel_name");
      if (localFunnel) {
        logger$8.debug(`Using persisted funnel from localStorage: ${localFunnel}`);
      ...
      if (persistedData) {
        ...
        if (parsed.state && parsed.state.funnel) {
          logger$8.debug(`Using persisted funnel from attribution: ${parsed.state.funnel}`);
    ...
    if (funnelMetaTag) {
      ...
      if (value) {
        logger$8.debug(`New funnel found from meta tag: ${value}`);
        try {
          sessionStorage.setItem("next_funnel_name", value);
          localStorage.setItem("next_funnel_name", value);
          logger$8.info(`Persisted funnel name: ${value}`);
        } catch (error) {
  ...
  handleEverflowClickId(metadata) {
    ...
    if (urlParams.has("evclid")) {
      ...
      logger$8.debug(`Everflow click ID found in URL: ${evclid}`);
    } else if (!evclid && sessionStorage.getItem("evclid")) {
      evclid = sessionStorage.getItem("evclid");
      if (evclid) {
        localStorage.setItem("evclid", evclid);
        logger$8.debug(`Everflow click ID found in sessionStorage: ${evclid}`);
      }
    }
    if (urlParams.has("sg_evclid")) {
      ...
      logger$8.debug(`SG Everflow click ID found: ${sg_evclid}`);
    } else {
    ...
    if (evclid) {
      ...
      logger$8.debug(`Added Everflow transaction ID to metadata: ${evclid}`);
    }
  }
  /**
  ...
  collectTrackingTags(metadata) {
    ...
    logger$8.debug(`Found ${trackingTags.length} tracking tags`);
    trackingTags.forEach((tag) => {
      ...
      if (tagName && tagValue) {
        ...
        logger$8.debug(`Added tracking tag: ${tagName} = ${tagValue}`);
        if (shouldPersist) {
          try {
            sessionStorage.setItem(`tn_tag_${tagName}`, tagValue);
            logger$8.debug(`Persisted tracking tag: ${tagName}`);
          } catch (error) {
  ...
  getFacebookPixelId() {
    ...
    if (pixelMeta) {
      ...
      if (pixelId) {
        logger$8.debug(`Facebook Pixel ID found from meta tag: ${pixelId}`);
        return pixelId;
      }
    }
    ...
    for (const script of scripts) {
      ...
      if (content.includes("fbq(") && content.includes("init")) {
        ...
        if (match && match[1]) {
          logger$8.debug(`Facebook Pixel ID found from script: ${match[1]}`);
          return match[1];
        }
      }
    }
...
class UserDataTracker {
  ...
  collectUserData() {
    ...
    try {
      ...
      logger$3.debug("Error accessing user data from storage:", error);
    }
    try {
