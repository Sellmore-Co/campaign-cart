  ...
  loadCampaign: async (apiKey) => {
    ...
    try {
      ...
      if (cachedData && cachedData.apiKey === apiKey && now - cachedData.timestamp < CACHE_EXPIRY_MS) {
        logger$d.info(`🎯 Using cached campaign data for ${currency} (expires in ` + Math.round((CACHE_EXPIRY_MS - (now - cachedData.timestamp)) / 1e3) + " seconds)");
      ...
      logger$d.info(`🌐 Fetching fresh campaign data from API with currency: ${currency}...`);
      const { ApiClient } = await import("./api-CMFTGFcZ.js");
      ...
      try {
        ...
        if (currency !== "USD") {
          logger$d.warn(`Failed to fetch campaign for ${currency}, falling back to USD:`, currencyError);
          const { useConfigStore: useConfigStore2 } = await Promise.resolve().then(() => configStore$1);
          const configStore22 = useConfigStore2.getState();
          configStore22.updateConfig({ selectedCurrency: "USD" });
          actualCurrency = "USD";
          campaign = await client.getCampaigns("USD");
          sessionStorage.removeItem("next_selected_currency");
          logger$d.info("✅ Successfully fetched campaign data with USD fallback");
        } else {
          throw currencyError;
        }
      }
...
class PendingEventsHandler {
  ...
  processPendingEvents() {
    ...
    logger$9.info(`Processing ${events.length} pending analytics events`);
    const processedIds = [];
