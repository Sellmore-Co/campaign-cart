...
const logger$e = createLogger("CampaignStore");
  ...
  loadCampaign: async (apiKey) => {
    ...
    try {
      ...
      if (!cachedData && requestedCurrency !== "USD") {
        ...
        if (cachedData) {
          logger$e.info(`No cache for ${requestedCurrency}, checking USD cache as potential fallback`);
        }
      }
      if (cachedData && cachedData.apiKey === apiKey && now - cachedData.timestamp < CACHE_EXPIRY_MS) {
        ...
        logger$e.info(`🎯 Using cached campaign data for ${cachedCurrency} (expires in ` + Math.round((CACHE_EXPIRY_MS - (now - cachedData.timestamp)) / 1e3) + " seconds)");
        if (cachedCurrency !== requestedCurrency) {
          logger$e.warn(`⚠️ Requested ${requestedCurrency} but using cached ${cachedCurrency} (fallback)`);
      ...
      logger$e.info(`🌐 Fetching campaign data from API with currency: ${requestedCurrency}...`);
      const { ApiClient } = await import("./api-C7AqiGNG.js");
      ...
      if (actualCurrency !== requestedCurrency) {
        logger$e.warn(`⚠️ API Fallback: Requested ${requestedCurrency}, received ${actualCurrency}`);
        configStore2.updateConfig({
      ...
      if (campaign.payment_env_key) {
        ...
        logger$e.info("💳 Spreedly environment key updated from campaign API: " + campaign.payment_env_key);
      }
      ...
      logger$e.info(`💾 Campaign data cached for ${actualCurrency} (10 minutes)`);
      if (actualCurrency !== requestedCurrency) {
        sessionStorage.removeItem(requestedCacheKey);
        logger$e.debug(`Cleared invalid cache for ${requestedCurrency}`);
      }
      set({
        data: campaign,
      ...
      if (!cartStore$1.isEmpty && cartStore$1.lastCurrency && cartStore$1.lastCurrency !== actualCurrency) {
        logger$e.info("Currency changed, refreshing cart prices...");
      ...
      logger$e.error("Campaign load failed:", error);
      throw error;
    }
  },
  getPackage: (id) => {
  ...
  clearCache: () => {
    try {
      ...
      keysToRemove.forEach((key) => {
        ...
        logger$e.debug(`Removed cache: ${key}`);
      });
      logger$e.info(`🗑️ Campaign cache cleared (${keysToRemove.length} entries removed)`);
    } catch (error) {
      logger$e.error("Failed to clear campaign cache:", error);
...
export {
  errorHandler$1 as $,
  trackCheckoutCompleted as A,
  trackCheckoutStarted as B,
  CountryService as C,
  trackCheckoutValidationFailed as D,
  EventBus as E,
  FieldFinder as F,
  trackCheckoutSubmitted as G,
  userDataStorage as H,
  getPaymentCapabilities as I,
  isApplePayAvailable as J,
  isPayPalAvailable as K,
  Logger as L,
  isGooglePayAvailable as M,
  trackEmptyCartCheckoutAttempt as N,
  getFailureUrl as O,
  PackageContextResolver as P,
  getSuccessUrl as Q,
  handleOrderRedirect as R,
  preserveQueryParams as S,
  TemplateRenderer as T,
  DisplayValueValidator as U,
  formatNumber as V,
  CurrencyFormatter as W,
  cartStore as X,
  AttributionCollector$1 as Y,
  index as Z,
  UtmTransfer$1 as _,
  useCampaignStore as a,
  DebugOverlay$1 as a0,
  analytics as a1,
  ...
  CART_STORAGE_KEY as l,
  trackCartLoaded as m,
  LogLevel as n,
  getCurrencySymbol as o,
  formatCurrency as p,
  ElementDataExtractor as q,
  PriceCalculator as r,
  nextAnalytics as s,
  trackAPICall as t,
  useCartStore as u,
  EcommerceEvents as v,
  ErrorDisplayManager as w,
  EventHandlerManager as x,
  trackDuplicateOrderPrevention as y,
  trackCheckoutFailed as z
};
