...
class EnhancerPanel {
  ...
  getEnhancedElements() {
    ...
    return Array.from(elements).map((element, index) => {
      ...
      const attributes = Array.from(htmlElement.attributes).filter((attr) => attr.name.startsWith("data-next-")).map((attr) => `${attr.name}="${attr.value}"`);
  ...
  getPerformanceStats(elements) {
    ...
    const mostComplex = elements.reduce(
      (max, el) => el.attributes.length > max.attributes ? { selector: el.selector, attributes: el.attributes.length } : max,
...
class APIPanel {
            ...
            ${recentRequests.map((request, _index) => `
              <div class="performance-bar">
                ...
                <div class="bar-container">
                  <div class="bar-fill" style="width: ${request.duration / 1e3 * 100}%"></div>
  ...
  exportRequests() {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `api-requests-${(/* @__PURE__ */ new Date()).toISOString()}.json`;
    a.click();
    URL.revokeObjectURL(url);
  }
  addRequest(request) {
...
class DebugOverlay {
  ...
  async createOverlay() {
    const { DebugStyleLoader } = await import("./DebugStyleLoader-B5ZWE-pv.js");
